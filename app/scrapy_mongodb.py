...
class MongoDBPipeline(BaseItemExporter):
    ...
    def create_un_idx(self, collection):
        # Ensure unique index
        if self.config['unique_key']:
            self.database[collection].ensure_index(self.config['unique_key'], unique=True)
            self.inited_collection[collection] = True
            log.msg(u'Ensuring index for key {0}'.format(
                self.config['unique_key']))
    ...
    def process_item(self, item, spider):
        ...
        collection = Util.get_collection_name(item.__class__.__name__)
        if not self.inited_collection.get(collection):
            self.create_un_idx(collection)
        ...
        if self.config['buffer']:
            ...
            if not self.item_buffer.has_key(collection):
                self.item_buffer[collection] = []

            self.item_buffer[collection].append(item)

            if len(self.item_buffer[collection]) >= self.config['buffer']:
                self.insert_item(self.item_buffer[collection][0: self.config['buffer']], spider, collection)
                self.item_buffer[collection] = self.item_buffer[collection][self.config['buffer']:]
            return item

        return self.insert_item(item, spider, collection)
    ...
    def close_spider(self, spider):
        ...
        """
        if self.item_buffer:
            for collection in self.item_buffer:
                self.insert_item(self.item_buffer[collection], spider, collection)

    def insert_item(self, item, spider, collection = None):
        """ Process the item and add it to MongoDB
        ...
        if self.config['unique_key'] is None:
            try:
                self.database[collection].insert(item, continue_on_error=True)
                log.msg(
            ...
            self.database[collection].update(key, item, upsert=True)
