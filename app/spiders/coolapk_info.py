...
class CoolpakInfoSpider(CrawlSpider):
    """
        酷安应用爬虫
    """
    name = "coolapk_info"
    ...
    rules = [
       ...
       #  Rule(LinkExtractor(allow=("http://apk\.hiapk\.com/games.*?sort=\d+&pi=\d+", )), callback='parse',follow=True),
        ...
        Rule(LinkExtractor(allow=("http://www\.coolapk\.com/game/([a-zA-Z0-9_\-]+\.)+[a-zA-Z0-9_\-]+", )), callback='parse_app',follow=True),
        Rule(LinkExtractor(allow=("http://www\.coolapk\.com/apk/([a-zA-Z0-9_\-]+\.)+[a-zA-Z0-9_\-]+", )), callback='parse_app',follow=True),
    ]
    ...
    def parse_app(self, response):
        ...
        try:
            ...
            item['name'] = response.css('div.ex-page-header div.container .media-body h1').xpath('text()').extract_first('')
            ...
            item['introduce'] = '<br/>'.join([x.strip() for x in response.css('.ex-card-content').xpath('.//text()').extract()]).strip('<br/>')
            item['category'] = category_dict.get(response.url.split('/')[-2])
            c_tags = response.css('div.row .col-md-5 .ex-card-content +div a')
            item['tags'] = ''
            # print item['introduce']
            # print ' '.join(response.css('div.row .col-md-5 .ex-card-content +div a').extract())
            for c_tag in c_tags:
                c_tag_name = c_tag.xpath('text()').extract_first('')
                c_tag_url = c_tag.xpath('@href').extract_first('')
                if c_tag_url.find('tag') >= 0:
                    item['tags'] += ' ' + c_tag_name
                else:
                    item['category'] += ' ' + c_tag_name

            item['imprint'] = ' '.join(response.css('.ex-card-content +div +div +div').xpath('text()').extract()).strip()
